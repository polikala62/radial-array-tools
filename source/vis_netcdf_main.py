...
def vis_ncdf(out_ncdf, pr_gdb, in_dem, pt_mask_fc, land_fc, xy_spacing, dist_range, densify_dist, obs_z_range, 
    ...
    check_paths_list = [i for i in [out_ncdf, pr_gdb, in_dem, pt_mask_fc, land_fc, sample_raster, landmark_fc] if i != ""]
    validate.check_paths(check_paths_list)
    ...
    if len(x_range) == 3 and len(y_range) == 3 and len(obs_z_range) == 3: #@TODO: better validation.
        
        # Get range values as variables.
        x_min, x_max, x_step = x_range
        y_min, y_max, y_step = y_range
        z_min, z_max, z_step = obs_z_range
    ...
    # Check densify_distance against distance range.
    validate.check_densify_dist(densify_dist, d_range)
